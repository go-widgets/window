...
func TestClippedAndClipFrames(t *testing.T) {
	rect := toolkit.Rect{X: 10, Y: 100, W: 80, H: 24}

	// The ordinary case: all of it shows. Wrapping this would cost a view per
	// control for nothing.
	if Clipped(rect, rect) {
		t.Error("a control that shows entirely was treated as clipped")
	}
	// Nothing shows: the descriptor already reports that as not Visible, and
	// hiding is cheaper than wrapping.
	for _, empty := range []toolkit.Rect{{}, {X: 10, Y: 100, W: 0, H: 24}, {X: 10, Y: 100, W: 80}} {
		if Clipped(rect, empty) {
			t.Errorf("an empty clip %v was treated as clipped", empty)
		}
	}
	// Half out of a viewport: the case nothing could honour.
	half := toolkit.Rect{X: 10, Y: 100, W: 80, H: 10}
	if !Clipped(rect, half) {
		t.Error("a control showing only its top was not treated as clipped")
	}

	outer, inner := ClipFrames(rect, half)
	if outer != half {
		t.Errorf("the clipping view is %v, want the visible rectangle %v", outer, half)
	}
	// The control keeps its SIZE and moves so the right part shows: squashed to
	// the sliver, a button would redraw its label to fit and look like another
	// button.
	if inner.W != rect.W || inner.H != rect.H {
		t.Errorf("the control was resized to %dx%d, want %dx%d", inner.W, inner.H, rect.W, rect.H)
	}
	if inner.X != 0 || inner.Y != 0 {
		t.Errorf("the control sits at %d,%d inside a view starting at its own corner", inner.X, inner.Y)
	}
	// Scrolled so its TOP is cut: the control moves up by what is hidden.
	cutTop := toolkit.Rect{X: 10, Y: 110, W: 80, H: 14}
	_, in2 := ClipFrames(rect, cutTop)
	if in2.Y != -10 {
		t.Errorf("with 10 hidden above, the control sits at y=%d, want -10", in2.Y)
	}
}
